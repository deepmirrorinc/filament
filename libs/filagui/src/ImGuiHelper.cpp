...
void ImGuiHelper::processImGuiCommands(ImDrawData* commands, const ImGuiIO& io) {
  ...
  for (size_t i = 0; i < mMaterialInstances.size(); i++) {
    mEngine->destroy(mMaterialInstances[i]);
  }
      ...
      if (pcmd.UserCallback) {
        ...
        rbuilder
            .geometry(primIndex, RenderableManager::PrimitiveType::TRIANGLES,
                      mVertexBuffers[bufferIndex], mIndexBuffers[bufferIndex], indexOffset,
                      pcmd.ElemCount)
  ...
  mVertexBuffers[bufferIndex]->setBufferAt(
      *mEngine, 0,
      VertexBuffer::BufferDescriptor(
          vbFilamentData, nVbBytes, [](void* buffer, size_t size, void* user) { free(buffer); },
          /* user = */ nullptr));
