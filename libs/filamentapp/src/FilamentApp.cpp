...
FilamentApp::FilamentApp() { initSDL(); }

FilamentApp::~FilamentApp() { SDL_Quit(); }
...
void FilamentApp::run(const Config& config, SetupCallback setupCallback,
                      ...
                      PreRenderCallback preRender, PostRenderCallback postRender, size_t width,
                      size_t height) {
  mWindowTitle = config.title;

  std::cout << "!!!!!!!!!!!!!!config.eye_position.z " << config.eye_position.z << std::endl;
  window = std::make_unique<FilamentApp::Window>(this, config, config.title, width, height);
  ...
  auto material = filament::Material::Builder()
                      ...
                      .build(*(mEngine));
  morbitPoint.reset(new Sphere(*(mEngine), material));
  morbitPoint->setRadius(0.0);
  // morbitPoint->getMaterialInstance()->setParameter("baseColor", filament::RgbaType::LINEAR,
  // filament::LinearColorA{0.5, 0.5, 0, 0.5});
  mScene->addEntity(morbitPoint->getSolidRenderable());
 ...
 window->mMainView->getView()->setShadowsEnabled(false);
  if (config.splitView) {
  ...
  if (imguiCallback) {
    mImGuiHelper =
        std::make_unique<ImGuiHelper>(mEngine, window->mUiView->getView(),
  ...
  while (!mClosed) {
    ...
    for (int i = 0; i < nevents; i++) {
      ...
      switch (event.type) {
        ...
        case SDL_KEYUP:
          ...
          break;
        case SDL_MOUSEWHEEL:
          if (!io || !io->WantCaptureMouse) window->mouseWheel(event.wheel.y);
          break;
        case SDL_MOUSEBUTTONDOWN:
        ...
        case SDL_MOUSEBUTTONUP:
          if (!io || !io->WantCaptureMouse) window->mouseUp(event.button.x, event.button.y);
          break;
        case SDL_MOUSEMOTION:
          if (!io || !io->WantCaptureMouse) window->mouseMoved(event.motion.x, event.motion.y);
          break;
        case SDL_DROPFILE:
          if (mDropHandler) {
    ...
    const float timeStep =
        mTime > 0 ? (float)((double)(now - mTime) / frequency) : (float)(1.0f / 60.0f);
    mTime = now;
    ...
    // cameraCube->mapFrustum(*mEngine, window->mDebugCamera);
...
void FilamentApp::getWindowSize(int* width, int* height) {
  SDL_GetWindowSize(window->getSDLWindow(), width, height);
}

filament::camutils::Manipulator<float>* FilamentApp::GetManipulator() {
  return window->mMainCameraMan;
}

filament::math::mat4f FilamentApp::GetCameraPose() {
  filament::math::mat4f pose = window->mMainCamera->getModelMatrix();
  return pose;
}
...
void FilamentApp::loadDirt(const Config& config) {
  if (!config.dirt.empty()) {
    ...
    mDirt =
        Texture::Builder().width(w).height(h).format(Texture::InternalFormat::RGB8).build(*mEngine);
...
FilamentApp::Window::Window(FilamentApp* filamentApp, const Config& config, std::string title,
  ...
  mMainCameraMan =
      CameraManipulator::Builder()
          .targetPosition(config.target_position.x, config.target_position.y,
                          config.target_position.z)
          .orbitHomePosition(config.eye_position.x, config.eye_position.y, config.eye_position.z)
          .zoomSpeed(2.0)
          .orbitSpeed(0.005, 0.01)
          //  .upVector(config.up_vec.x,config.up_vec.y,config.up_vec.z)
          .groundPlane(0, 1, 0, 0)
          .flightMoveDamping(15.0)
          .build(config.cameraMode);
  mDebugCameraMan =
      CameraManipulator::Builder().targetPosition(0, 0, 0).build(camutils::Mode::ORBIT);
  ...
  mMainCamera->lookAt(config.eye_position, config.target_position, config.up_vec);
}
...
void FilamentApp::Window::mouseDown(int button, ssize_t x, ssize_t y) {
  ...
  for (auto const& view : mViews) {
    if (view->intersects(x, y)) {
      ...
      filament::LinearColorA color{0.5, 0.5, 0, 0.5};
      mFilamentApp->morbitPoint->getMaterialInstance()->setParameter(
          "baseColor", filament::RgbaType::LINEAR, color);
      mFilamentApp->morbitPoint->setRadius(0.1);
      mFilamentApp->morbitPoint->setPosition(mMainCameraMan->GetGrapPoint());
      break;
    }
  }
}
...
void FilamentApp::Window::mouseUp(ssize_t x, ssize_t y) {
  ...
  filament::LinearColorA color{0.5, 0.5, 0, 0};
  mFilamentApp->morbitPoint->setRadius(0.0);
  mFilamentApp->morbitPoint->getMaterialInstance()->setParameter("baseColor",
                                                                 filament::RgbaType::LINEAR, color);
}
...
void FilamentApp::Window::configureCamerasForWindow() {
  ...
  double near = 0.1;
  double far = 100;
  // mMainCamera->setLensProjection(mFilamentApp->mCameraFocalLength, double(mainWidth) / height,
  // near,
  //                                far);
  mMainCamera->setProjection(45.0, double(width) / height, 0.0625, 4096, Camera::Fov::VERTICAL);
  ...
  mUiCamera->setProjection(Camera::Projection::ORTHO, 0.0, width / dpiScaleX, height / dpiScaleY,
                           0.0, 0.0, 1.0);
...
FilamentApp::CView::~CView() { engine.destroy(view); }
...
void FilamentApp::CView::setCameraManipulator(CameraManipulator* cm) { mCameraManipulator = cm; }

void FilamentApp::CView::setCamera(Camera* camera) { view->setCamera(camera); }
